module.exports = function (shoppingcart,knex){
  ...
  shoppingcart.get('/generateUniqueId',async(request, response, next)=>{
    var uniqueId = generate_unique_id()
    var query = await request.db.ShoppingCart.query().select('cart_id').where('cart_id',uniqueId).then((cart)=>{
      if(cart.length == 0){
  ...
  // Add product in cart
  // NOTE: Before creating any new cart first generate unique cart id
  shoppingcart.post('/add',async(request, response, next)=>{
      ...
      var insertQuery = await request.db.ShoppingCart.query().insert(shoppingCart_details).then(async()=>{
        var query = await request.db.ShoppingCart.query().select(
          'item_id',
          'product.name',
          'attributes',
          'product.product_id',
  ...
  shoppingcart.get('/:cart_id',async(request, response, next)=>{
  ...
  //Update cart by item
  shoppingcart.put('/update/:item_id',async(request, response, next)=>{
      //Example for request body
      `{
      	 "attributes" : "L",
         "quantity" : 2
      ...
      var shoppingCart_details = request.body;
      var updateQuery = await request.db.ShoppingCart.query().patch(shoppingCart_details).where('item_id',item_id).then(async()=>{
        var query = await request.db.ShoppingCart.query().select(
          'item_id',
          'product.name',
          'attributes',
          ...
          'price',
          'quantity'
        ).join('product','shopping_cart.product_id','=','product.product_id').where('item_id',item_id)
        .then((shoppingCart)=>{
          for(let cart of shoppingCart){
            cart['subtotal'] = Number(cart.price)* Number(cart.quantity);
          };
  ...
  //Empty cart
  shoppingcart.delete('/empty/:cart_id',async(request, response, next)=>{
    var cart_id = request.params.cart_id;
    var deleteQuery = await request.db.ShoppingCart.query().delete().where('cart_id',cart_id).then(async()=>{
      var query = await request.db.ShoppingCart.query().then((emptyCart)=>{
  ...
  shoppingcart.get('/totalAmount/:cart_id',async(request, response, next)=>{
    var cart_id = request.params.cart_id;
    var query = await request.db.ShoppingCart.query().select('product.price')
    .join('product','shopping_cart.product_id','=','product.product_id')
  ...
  shoppingcart.get('/getSaved/:cart_id',async(request, response, next)=>{
      var cart_id = request.params.cart_id;
      var query = await request.db.ShoppingCart.query().from('save_items')
      .where('cart_id',cart_id)
      .then((savedItem)=>{
        console.log("\nGet save items:\n",savedItem[0]);
  ...
  shoppingcart.delete('/removeProduct/:item_id',async(request, response, next)=>{
    var item_id = request.params.item_id;
    var removeQuery = await request.db.ShoppingCart.query().delete().where('item_id',item_id)
    .then(()=>{
